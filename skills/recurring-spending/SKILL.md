...
- **Account** = the expense account the charge posts to, as the full account name (e.g. `Expenses:Utilities`).
- **Notes** = short flags like "amount varies" or "merged from 3 spellings"; leave the cell empty when there's nothing to note.
...
- **Coming up**: list the payments expected in the next 30 days, ordered by date (next expected = last charge date + cadence).
